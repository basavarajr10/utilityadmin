    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.notifications.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('channel') ? 'is-invalid' : '' }}" name="channel" id="channel" required>
                    ...
                    @foreach(App\Models\Notification::CHANNEL_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('channel', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('audience') ? 'is-invalid' : '' }}" name="audience" id="audience">
                    ...
                    @foreach(App\Models\Notification::AUDIENCE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('audience', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sent_count') ? 'is-invalid' : '' }}" type="text" name="sent_count" id="sent_count" value="{{ old('sent_count', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('delivered_count') ? 'is-invalid' : '' }}" type="text" name="delivered_count" id="delivered_count" value="{{ old('delivered_count', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('sent_at') ? 'is-invalid' : '' }}" type="text" name="sent_at" id="sent_at" value="{{ old('sent_at') }}">
