...
<span>{{ trans('cruds.billTransaction.title_singular') }} {{ trans('global.list') }}</span>
@can('bill_transaction_create')
<a href="{{ route('admin.bill-transactions.create') }}" class="btn-add-record"><i class="fas fa-plus"></i> {{ trans('global.add') }} {{ trans('cruds.billTransaction.title_singular') }}</a>
@endcan
</div>

    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-BillTransaction">
            <thead>
                <tr>
                    <th width="10">
                ...
                <tr>
                    ...
                    <td>
                        <select class="search" strict="true">
                            <option value>{{ trans('global.all') }}</option>
                            @foreach(App\Models\BillTransaction::CATEGORY_SELECT as $key => $item)
                    ...
                    <td>
                        <select class="search" strict="true">
                            <option value>{{ trans('global.all') }}</option>
                            @foreach(App\Models\BillTransaction::STATUS_SELECT as $key => $item)
                                <option value="{{ $key }}">{{ $item }}</option>
                            @endforeach
                        </select>
                    </td>
...
@endcan
  ...
  let dtOverrideGlobals = {
    ...
    retrieve: true,
    aaSorting: [],
    ajax: "{{ route('admin.bill-transactions.index') }}",
    columns: [
...
{ data: 'actions', name: '{{ trans('global.actions') }}' }
  ...
  let table = $('.datatable-BillTransaction').DataTable(dtOverrideGlobals);
